...
class parseDBArtistsData:
    ...
    def parseArtistsAM(self, modVal):
        if self.primary is True:
            self.dbdata["AllMusic"]["Artists"].parseArtistModValFiles(modVal, force=self.force, previousDays=self.previousDays)
        if self.credit is True:
    ...
    def parseArtistsMB(self, modVal):
        if self.primary is True:
            self.dbdata["MusicBrainz"]["Artists"].parseArtistModValFiles(modVal, force=self.force, previousDays=self.previousDays)
        if self.extra is True:
    ...
    def parseArtistsLM(self, modVal):
        if self.primary is True:
            self.dbdata["LastFM"]["Artists"].parseArtistModValFiles(modVal, force=self.force, previousDays=self.previousDays)
        if self.extra is True:
    ...
    def parse(self, db, nProcs=8, modVals=range(100), previousDays=5, force=None, primary=True, extra=False, credit=False):
        ...
        print("DB: {0} --> Primary={1}   Extra={2}   Credit={3}   PreviousDays={4}".format(db, self.primary, self.extra, self.credit, self.previousDays))
